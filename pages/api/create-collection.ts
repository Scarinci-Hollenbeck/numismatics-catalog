...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const existingCollection: Array<ICollections> = await Collections.find({ title: results.category }).limit(1);
      ...
      if (existingCollection.length > 0) {
        ...
        res.status(201).json({ status: 201, newCollection: true, data: newCollection._id });
      }
    } catch (error) {
      console.error(error);
